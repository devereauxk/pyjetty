...
class ProcessMC_JetTrk(process_mc_base.ProcessMCBase):
  ...
  def initialize_user_output_objects_R(self, jetR):
    ...
    for observable in self.observable_list:
      ...
      for trk_thrd in self.obs_settings[observable]:
        ...
        self.jet_type_labels = ["Truth", "Det", "DetEmb", "UE"] # UE / background??
        # all the same but matched?

        for type_label in self.jet_type_labels:
        
          if observable == 'jet_pt_JetPt':
            name = 'h_{}_R{}_{}_{}'.format(observable, jetR, obs_label, type_label)
        ...
        if observable == 'deltajetpt_JetPt':
            name = 'h_{}_R{}_{}_{}'.format(observable, jetR, obs_label, type_label)
            deltajetpt_bins = linbins(0,50,200)
            jetpt_bins = linbins(0,200,200)
            h = ROOT.TH2D(name, name, 200, deltajetpt_bins, 200, jetpt_bins)
            h.GetXaxis().SetTitle('#Delta p_{T jet}')
  ...
  def fill_matched_jet_histograms(self, jet_det, jet_det_groomed_lund, jet_truth,
    ...
    for observable in self.observable_list:
      ...
      if observable == "deltajetpt_JetPt":
        h = getattr(self, "h_{}_R{}_{}".format(observable, jetR, obs_label))
        h.Fill(jet_det.perp() - jet_pp_det.perp(), jet_pp_det.perp())
